...
function Music(title, artist,favorite, uuid){
    this.title = title
    this.artist = artist
    ...
    if(target == formBtn){
        ...
        if(displayedUser.innerText != ''){
            ...
            if(validation == true){
                ...
                const newSong = new Music(title, author, readRadio, uuid);
                if(newSong.favorite == 'Yes'){
                    newSong.favorite = 'true';
                }else if(newSong.favorite == 'No'){
                    newSong.favorite = 'false';
                }
                console.log(newSong);
                addNewMusic(newSong, displayedUser.innerText);
                books.push(newSong);
                renderBooks(newSong, addBookContainer, displayedUser);
                overlayForm.reset();
        }else{
            event.preventDefault();
...
function removeBook(id,bookName,username){
    ...
    axios.delete(serverApi+`/delete-music/${bookName}/${username}`).then((response)=>{
...
function readButton(id, songName, username, condition){
    ...
    axios.put(serverApi+`/update-music/${songName}/${username}/${condition}`).then((response)=>{
        if(response.data == 'Read Value updated'){
            readButtonCard.setAttribute('onclick', `readButton(`+id+`,`+`"`+songName+`"`+`,`+`"`+username+`"`+`,`+`"false")`);
        }else if(response.data == 'Read Value updated-2'){
            readButtonCard.setAttribute('onclick', `readButton(`+id+`,`+`"`+songName+`"`+`,`+`"`+username+`"`+`,`+`"true")`);
        }
    }).catch(error => {
        console.log(error);
    });
    ...
    if(user) {
        axios.get(serverApi+`/login/${user.username}/${user.password}`).then(response => {
            if(response.data != null){
                ...
                axios.get(serverApi+`/music-load/${user.username}/${user.password}`).then(response =>{
...
function getExistingUser(username, password, signUp, logIn, logOut, displayedUser, container){
    axios.get(serverApi+`/login/${username}/${password}`).then(response => {
        if(response.data == `User ${username} Exists`){
            ...
            axios.get(serverApi+`/music-load/${username}/${password}`).then(response =>{
                for(let i=0;i<response.data.length;i++){
                    renderBooks(response.data[i], container, displayedUser);
                }
            });
...
function addNewMusic(createdBook, username){
    axios.post(serverApi+`/music/${username}`, createdBook).then(response => {
        if(response.data == true){
            alert(`${username} Song added successfully`);
...
function renderBooks(response, container, displayedUser){
    ...
    authorH5.innerText = response.artist;
